...
export default {
  ...
  created() {
    axios
      .get(`/blog/editpost/${this.$route.params.blogPostId}`)
      .then(response => {
        this.form.fill(response.data.blog_post);
      });
  },
  computed: {
    allCategories() {
  ...
  methods: {
    ...
    updatePost() {
      this.form
        .post(`/blog/update-post/${this.$route.params.blogPostId}`)
        .then(response => {
          this.$router.push("/dashboard/blog/post-list2");
          Toast.fire({
            type: "success",
    ...
    updateImage() {
      if (this.form.photo) {
        ...
        if (img.length > 100) {
          ...
          return `/images/post_images/blog/${this.form.photo}`;
        }
      }
    }
  }
